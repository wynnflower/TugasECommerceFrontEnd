...
import {onLogin}from './../1.actions' // nama functionnya
...
class Login extends React.Component{
    //Ke Trigger kalau ada perubahan props, yaitu global state (contoh username dari '' menjadi 'piko')
    componentWillReceiveProps(newProps){
        // alert('masuk')
        Cookie.set('userData',newProps.username,{path:'/'})
    }
    onBtnLoginClick=()=>{
        var username=this.refs.username.value // piko
        var password=this.refs.password.value // rahasia123
        this.props.onLogin(username,password) // Action Creator dari userAction.js (berupa function 2 parameter)
        // Cookie.set() --> juga bisa
    }
    renderBtnOrLoading=()=>{
